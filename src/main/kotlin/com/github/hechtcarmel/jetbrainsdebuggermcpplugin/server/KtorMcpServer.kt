...
class KtorMcpServer(
    ...
    private fun Application.configureRouting() {
        routing {
            // GET /debugger-mcp/sse → Open SSE stream
            get(McpConstants.SSE_ENDPOINT_PATH) {
                handleSseRequest(call)
            }

            // POST /debugger-mcp → JSON-RPC (Streamable HTTP or with sessionId for SSE)
            post(McpConstants.MCP_ENDPOINT_PATH) {
                handlePostRequest(call)
            }

            // POST /debugger-mcp/sse → JSON-RPC (some clients POST to SSE endpoint)
    ...
    /**
     * Handles POST /debugger-mcp - JSON-RPC requests.
     *
     * Supports two modes:
